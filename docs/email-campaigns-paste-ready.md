# Email Campaigns — Paste-Ready

5 plain-text emails for the AC Certified Consultant nurture, formatted to copy
straight into ActiveCampaign's campaign builder.

---

## One-time setup in AC (before pasting any email)

1. **Set a default for First Name field** so `%FIRSTNAME%` never renders empty:
   - Contacts → Manage Fields → First Name → Set default value: `there`
   - Result: contacts with no first name receive "Hi there," instead of "Hi ,"

2. **Per-campaign settings to apply to all 5:**
   - From name: `Greg Getner`
   - From email: `greg@getner.ai`
   - Reply-to: `greg@getner.ai`
   - Send to list: `ac-cert-leads`
   - Format: **Plain text** (not HTML — important for the 1:1 promise)
   - Open tracking: **OFF**
   - Link tracking: **OFF**
   - Google Analytics: OFF

3. **Workflow per email:** Campaigns → New → Standard → Plain text →
   paste subject + preheader + body → Save as draft. Don't schedule yet —
   the automation will trigger them.

---

## Email 1 — Day 0 (immediate after list add)

**Campaign name (internal):** `AC Cert Leads — Email 1 — Day 0 — Welcome`

**SUBJECT:**
```
Got your note — quick question before we talk
```

**PREHEADER:**
```
Two-minute form so I don't waste your first 30 minutes.
```
...
## Email 2 — Day 1 (24h after Email 1, only if not booked)

**Campaign name (internal):** `AC Cert Leads — Email 2 — Day 1 — DwY vs DfY`

**SUBJECT:**
```
DwY vs DfY — which one are you?
```

**PREHEADER:**
```
A short framework so you don't have to ask the question on the call.
```
...
**Campaign name (internal):** `AC Cert Leads — Email 3 — Day 3 — 5–10 Rule`

**SUBJECT:**
```
The 5–10% rule (no ask in this one)
```

**PREHEADER:**
```
Just a quick read on how I think about AC accounts.
```
...
**Campaign name (internal):** `AC Cert Leads — Email 4 — Day 7 — Client Outcomes`
...
**PREHEADER:**
```
Two real outcomes from the audit. Then I'll stop emailing for a week.
```
...
> ⚠️ **Heads-up before sending Email 4:** the two client outcomes (course creator
> and business coach) were placeholder numbers from the original brief, not real
> attributed clients. Your project memory flagged this for replacement before
> driving real traffic. Either swap in two real anonymized client outcomes or
> rewrite as a generic pattern observation.

---

## Email 5 — Day 14 (final, only if not booked)

**Campaign name (internal):** `AC Cert Leads — Email 5 — Day 14 — Closing Loop`
...
**PREHEADER:**
```
Last email — I'll get out of your inbox.
```
...
## After all 5 are saved as drafts

Build the automation in AC's visual builder per `/docs/ac-cert-leads-automation.md`.
Trigger: tag added = `consult-form-submitted`. Each step uses "Send a campaign"
pointing at the matching draft above. Wait conditions are 1 / 3 / 7 / 14 days
between sends. Goal step: any Calendly booking → exit + tag `consult-booked`.
Reply-detected exit: `replied-in-sequence` tag → exit (manual tagging until you
wire reply detection).
